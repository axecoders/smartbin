...
(function() {
  ...
  function doTsp(mode) {
    ...
    if (numActive <= maxTspBF + mode) {
      //alert(numActive+"<="+maxTspBF+"+"+mode);
      tspBruteForce(mode, 0, 0, 1);
    } else if (numActive <= maxTspDynamic + mode) {
   //alert(numActive+"<="+maxTspDynamic+"+"+mode);
        tspBruteForce(mode, 0, 0, 1);
      //tspDynamic(mode);
    } else {
    alert("Sorry for the inconvinience as route cannot be generated at this moment, please check with support team");
      //tspAntColonyK2(mode);
      //tspK3(mode);
    }
